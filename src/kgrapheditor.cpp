...
void KGraphEditor::optionsConfigure()
{
  ...
  qDebug() << KGraphEditorSettings::parsingMode();
...
void KGraphEditor::slotEditNewVertex()
{
  if (m_currentPart == 0)
  {
    qDebug() << "new vertex: no part selected";
    return;
  }
  qDebug() << "new vertex";
...
void KGraphEditor::slotEditNewEdge()
{
  if (m_currentPart == 0)
  {
    qDebug() << "new edge: no part selected";
    return;
  }
  qDebug() << "new edge";
