...
bool DotGraph::parseDot(const QString& str)
{
  ...
  qDebug() << "Running " << m_layoutCommand  << str;
  QStringList options;
...
//   }
  ...
  qDebug() << "m_dot is " << m_dot  << ". Acquiring mutex";
...
void DotGraph::slotDotRunningDone(int exitCode, QProcess::ExitStatus exitStatus)
{
  ...
  qDebug() << "string content is:" << endl << result << endl << "=====================" << result.size();
...
//   }
  ...
  if (parsingResult)
  {
    qDebug() << "calling updateWithGraph";
...
//       int cellNum = cellNumber(int(gn->x()), int(gn->y()));
      ...
      qDebug() << "Found cell number " << cellNum;
      ...
      {
        qDebug() << "cell number " << cellNum  << " contains " << m_cells[cellNum].size() << " nodes";
        ...
        qDebug() << "cell factor is now " << m_horizCellFactor << " / " << m_vertCellFactor;
        stop = false;
        break;
      }
    }
  } while (!stop);
  qDebug() << "m_wdhcf=" << m_wdhcf << "; m_hdvcf=" << m_hdvcf << endl;
...
void DotGraph::updateWithGraph(graph_t* newGraph)
{
  ...
  while(attr)
  {
    qDebug() << agnameof(newGraph) << ":" << attr->name << agxget(newGraph,attr);
  ...
  qDebug() << "first node:" << (void*)ngn;
  
  while (ngn != NULL)
//   foreach (GraphNode* ngn, newGraph.nodes())
  {
    qDebug() << "node " << agnameof(ngn);
...
void DotGraph::updateWithGraph(const DotGraph& newGraph)
{
  ...
  {
    qDebug() << "subgraph" << nsg->id();
    if (subgraphs().contains(nsg->id()))
    {
      qDebug() << "subgraph known" << nsg->id();
...
{
  qDebug() << subgraphName << " from " << subgraphs().keys();
  GraphSubgraph* subgraph = subgraphs()[subgraphName];
  ...
  foreach(GraphElement* element, subgraph->content())
  {
    ...
    {
      qDebug() << "Adding" << element->id() << "to main graph";
...
void DotGraph::addNewNode(QMap<QString,QString> attribs)
{
  ...
  qDebug() << "node added as" << newNode->id();
}
...
void DotGraph::addExistingNodeToSubgraph(QMap<QString,QString> attribs,QString subgraph)
{
  ...
  if (nodes().contains(attribs["id"]))
  {
    ...
    qDebug() << "node " << node->id() << " added in " << subgraph;
  }
  else
  {
    foreach(GraphSubgraph* gs, subgraphs())
    {
      ...
      if (elt != 0)
      {
        qDebug() << "removing node " << elt->id() << " from " << gs->id();
        gs->removeElement(elt);
        subgraphs()[subgraph]->content().push_back(elt);
        qDebug() << "node " << elt->id() << " added in " << subgraph;
        break;
      }
    }
  }
...
void DotGraph::moveExistingNodeToMainGraph(QMap<QString,QString> attribs)
{
  ...
  else
  {
    foreach(GraphSubgraph* gs, subgraphs())
    {
      ...
      if (found)
      {
        qDebug() << "removing node " << node->id() << " from " << gs->id();
        gs->removeElement(node);
        nodes()[node->id()] = node;
        qDebug() << "node " << node->id() << " moved to main graph";
        break;
      }
    }
  }
...
void DotGraph::renameNode(const QString& oldNodeName, const QString& newNodeName)
{
  ...
  {
    qDebug() << "Renaming " << oldNodeName << " into " << newNodeName;
