  ...
  , m_command(command)
{
	...
	// settings
//! @todo default?
	m_unit = QLocale::system().measurementSystem() == QLocale::MetricSystem ? KgvUnit::U_CM : KgvUnit::U_INCH;
...
// 	m_contents->setFocusProxy(m_contents->headerTitleLineEdit);
  ...
  m_contents->iconLabel->setPixmap(QIcon::fromTheme("distribute-horizontal-page").pixmap(32, 32));
...
{
  qDebug() << "KGVSimplePrintingPageSetup::slotFittingButtonClicked " << id;
  if (id == NaturalSize)
  {
