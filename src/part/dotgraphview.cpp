...
void DotGraphViewPrivate::updateBirdEyeView()
{
  ...
  if (zp == KGraphViewerInterface::Auto)
  {
    ...
    qDebug() << tr1Pos.x() << tr1Pos.y() << tr2Pos.x() << tr2Pos.y();
...
int DotGraphViewPrivate::displaySubgraph(GraphSubgraph* gsubgraph, int zValue, CanvasElement* parent)
{
  ...
  {
    qDebug() << "Creating canvas subgraph for" << gsubgraph->id();
    ...
    qDebug() << " one CanvasSubgraph... Done";
  }
  foreach (GraphElement* element, gsubgraph->content())
  {
    ...
    {
      qDebug() << "Creating canvas node for:" << gnode->id();
...
void DotGraphViewPrivate::setupPopup()
{
  ...
  qDebug() << "DotGraphView::setupPopup";
  m_popup = new QMenu();
  ...
  qDebug() << "    m_bevEnabledAction setting checked to : " << KGraphViewerPartSettings::birdsEyeViewEnabled();
...
{
  //qDebug() << "New node pic=" << KGlobal::dirs()->findResource("data","kgraphviewerpart/pics/kgraphviewer-newnode.png");
  Q_D(DotGraphView);
  d->m_canvas = 0;
...
bool DotGraphView::slotLoadLibrary(graph_t* graph)
{
  ...
  qDebug() << "layoutCommand:" << layoutCommand;
  ...
  QGraphicsScene* newCanvas = new QGraphicsScene();
  qDebug() << "Created canvas " << newCanvas;
...
//   kDebug() << "sceneRect is now " << m_canvas->sceneRect();
  ...
  qDebug() << "Creating" << d->m_graph->subgraphs().size() << "CanvasSubgraphs from" << d->m_graph;
  int zvalue = -1;
  ...
  qDebug() << "Creating" << d->m_graph->nodes().size() << "nodes from" << d->m_graph;
  GraphNodeMap::const_iterator it = d->m_graph->nodes().constBegin();
  for (; it != d->m_graph->nodes().constEnd();it++)
  {
    const QString& id = it.key();
    GraphNode* gnode = it.value();
    qDebug() << "Handling" << id << (void*)gnode;
    qDebug() << "  gnode id=" << gnode->id();
    qDebug()<<  "  canvasNode=" << (void*)gnode->canvasNode();
    if (gnode->canvasNode()==0)
    {
      qDebug() << "Creating canvas node for" << gnode->id();
...
//       cnode->setZValue(gnode->z());
  ...
  qDebug() << "Creating" << d->m_graph->edges().size() << "edges from" << d->m_graph;
  foreach (GraphEdge* gedge, d->m_graph->edges())
  {
    qDebug() << "One GraphEdge:" << gedge->id();
    if (gedge->canvasEdge() == 0
      && gedge->fromNode() != 0
      && gedge->toNode() != 0)
    {
      qDebug() << "New CanvasEdge for" << gedge->id();
      qDebug() << "edge fromNode=" << (void*)gedge->fromNode();
      qDebug() << "              "<< gedge->fromNode()->id();
      qDebug() << "edge toNode=" << (void*)gedge->toNode();
      qDebug() << "              "<< gedge->toNode()->id();
  ...
  qDebug() << "Adding graph render operations: " << d->m_graph->renderOperations().size();
...
//   setDragMode(NoDrag);
  ...
  else if (d->m_editingMode == SelectingElements)
  {
    ...
    qDebug() << "Stopping selection" << scene() << d->m_canvas;
...
void DotGraphView::slotSelectLayoutAlgo(const QString& ttext)
{
  ...
  qDebug() << "DotGraphView::slotSelectLayoutAlgo '" << text << "'";
  if (text == "Dot")
  {
...
{
  Q_D(DotGraphView);
  qDebug() << "DotGraphView::slotBevToggled";
  qDebug() << "    d->m_bevEnabledAction is checked ? " << d->m_bevEnabledAction->isChecked();
...
void DotGraphView::createNewEdgeDraftFrom(CanvasElement* node)
{
  ...
  if (d->m_newEdgeDraft != 0)
  {
    qDebug() << "removing new edge draft";
    d->m_newEdgeDraft->hide();
    scene()->removeItem(d->m_newEdgeDraft);
    delete d->m_newEdgeDraft;
    d->m_newEdgeDraft = 0;
...
void DotGraphView::removeSelectedEdges()
{
  ...
  foreach(GraphEdge* e, d->m_graph->edges())
  {
    if (e->isSelected())
    {
      qDebug() << "emiting removeEdge " << e->id();
...
void DotGraphView::removeSelectedNodes()
{
  ...
  foreach(GraphNode* e, d->m_graph->nodes())
  {
    if (e->isSelected())
    {
      qDebug() << "emiting removeElement " << e->id();
